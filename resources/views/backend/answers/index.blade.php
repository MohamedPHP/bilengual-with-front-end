                                                        ...
                                                        @if (count(App\Answer::where('question_id', $question->id)->get()) > 0)
                                                            <ul class="list-unstyled">
                                                                ...
                                                                @foreach (App\Answer::where('question_id', $question->id)->get() as $answer)
                                                                    <li class="answer">
                                                                        <div class="row">
                                                                            <div class="col-md-5" data-qid="{{$question->id}}" data-aid="{{ $answer->id }}">
                                                                                {{$answer->content}}
                                                                            </div>
                                                                            @if ($answer->result == 1)
                                                                                <div class="col-md-3">
                                                                            ...
                                                                            <div class="col-md-4 pull-right">
                                                                                ...
                                                                                <a href="{{ route('answer.delete', ['id' => $answer->id]) }}" class="btn btn-sm btn-danger">Delete</a>
                                                                            </div>
                                                                        </div>
                                                                    </li>
                                                                @endforeach
                ...
                <form action="{{ route('answers.create') }}" method="post">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="Quize">Answers Content</label>
                ...
                <form action="{{ route('answer.update') }}" method="post">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                ...
                                <input type="text" name="answer_content" id="answer_content" placeholder="answer_content" class="form-control" style="background-color:#fff; box-shadow: 3px 3px 10px #ccc;">
                            </div>
                            <div class="form-group">
...
@section('scripts')
    <script type="text/javascript">
        ...
        var questionAnswerElement = null;

        $('.answer_singel').click(function (event) {
            event.preventDefault();

            //set data in vars
            answerElement = event.target.parentNode.parentNode.childNodes[1];
            questionAnswerElement = event.target.parentNode.parentNode.childNodes[1];
            id = event.target.parentNode.parentNode.childNodes[1];

            // get the text content
            var answer_content = answerElement.textContent.trim();
            var question_of_answer = questionAnswerElement.dataset['qid'];
            var id = questionAnswerElement.dataset['aid'];


            $('#answer_content').val(answer_content);
            $('#answer_question').val(question_of_answer);
            $('#id').val(id);
