                                                                    ...
                                                                    <li class="answer">
                                                                        <div class="row" data-qid = "{{ $question->id }}" data-aid="{{ $answer->id }}">
                                                                            ...
                                                                            <div class="col-md-4 pull-right">
                                                                                ...
                                                                                <a href="{{ route('answer.question-text.answer-voice.delete', ['id' => $answer->id]) }}" class="btn btn-sm btn-danger">Delete</a>
                                                                            </div>
                                                                        </div>
                                                                    </li>
                                                                @endforeach
    ...
    <div class="modal fade" tabindex="-1" role="dialog" aria-labelledby="gridSystemModalLabel" id="Answers_Modal">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('answers.question-text.answer-voice.create') }}" method="post" enctype="multipart/form-data">
                    {{ csrf_field() }}
    ...
    <div class="modal fade" tabindex="-1" role="dialog" aria-labelledby="gridSystemModalLabel" id="Edit_Single_Answer_Modal">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('answer.question-text.answer-voice.update') }}" method="post" enctype="multipart/form-data">
                    {{ csrf_field() }}
                    <input type="hidden" name="id" id="id">
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="">Answer Content</label>
                            <input type="file" name="answer_vioce_question_text_content" id="answer_vioce_question_text_content" placeholder="answer_content" class="form-control" style="background-color:#fff; box-shadow: 3px 3px 10px #ccc;" value="sdftgdsrgsdghdryh">
                        </div>
                        <div class="form-group">
                            <label>Question</label>
                            <select name="answer_voice_question_text_question" id="answer_voice_question_text_question" class="form-control" style="background-color:#fff; box-shadow: 3px 3px 10px #ccc;">
                                <option value="">---------- Select A Question ----------</option>
                                @foreach ($questions as $question)
                                    <option value="{{ $question->id }}">{{ $question->content }}</option>
                                @endforeach
...
@section('scripts')
    <script type="text/javascript">
        ...
        $('.answer_singel').click(function (event) {
            ...
            var Question_id = event.target.parentNode.parentNode.dataset['qid'];
            var Answer_id = event.target.parentNode.parentNode.dataset['aid'];

            $('#id').val(Answer_id);
            $('#answer_voice_question_text_question').val(Question_id);
